...
const PatrimonioMobileCard: React.FC<{
    ...
    return (
        <div className={`p-4 transition-colors duration-200 ${deletingId === entry.id ? 'opacity-50 pointer-events-none' : 'opacity-100'} ${newlyAddedId === entry.id ? 'bg-neutral-100 dark:bg-neutral-800/50' : isExpanded ? 'bg-neutral-50 dark:bg-neutral-800/30' : ''}`}>
            <div className="flex items-start justify-between gap-4">
                <div className="flex-grow cursor-pointer" onClick={toggleExpand}>
                    <div className="flex items-center gap-2">
                        <p className="font-semibold">{entry.data.slice(0, 7)}</p>
                        <ChevronDown className={`h-4 w-4 text-neutral-400 transition-transform duration-300 ${isExpanded ? 'rotate-180' : ''}`} />
                    </div>
                    <p className="font-bold text-lg font-mono tracking-tight text-black dark:text-white mt-1">{config.simbolo_moeda}{entry.total_patrimonio.toFixed(2)}</p>
                    {entry.variation !== null ? (
                        <p className={`text-sm font-semibold font-mono tracking-tight mt-0.5 ${entry.variation >= 0 ? 'text-green-600 dark:text-green-500' : 'text-red-600 dark:text-red-500'}`}>
                            {entry.variation >= 0 ? '+' : ''}{config.simbolo_moeda}{entry.variation.toFixed(2)}
                        </p>
            ...
            <div ref={contentRef} className="collapsible-content" style={{ maxHeight: isExpanded ? contentRef.current?.scrollHeight : 0 }}>
